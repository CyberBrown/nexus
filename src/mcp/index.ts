...
export function createNexusMcpServer(env: Env, tenantId: string, userId: string) {
  ...
  server.tool(
    ...
    'Execute a queued task immediately via sandbox-executor. Routes claude-ai tasks to /execute/sdk (fast AI path) and claude-code tasks to /execute (container path). Also supports de-agent tasks via DE service. Use this for immediate task execution instead of waiting for the 15-minute cron.',
    {
      ...
      branch: z.string().optional().describe('Branch to commit to (default: "main"). For claude-code tasks only.'),
      commit_message: z.string().optional().describe('Custom commit message. For claude-code tasks only.'),
      passphrase: passphraseSchema,
    },
    async (args): Promise<CallToolResult> => {
      // Validate passphrase for write operation
  ...
  server.tool(
    ...
    'Create a new task in Nexus. Use auto_dispatch=true to immediately queue it for execution instead of waiting for the 15-minute cron.',
    {
      ...
      project_id: z.string().uuid().optional().describe('Project ID to associate this task with'),
      auto_dispatch: z.boolean().optional().describe('If true and status is "next", immediately queue for execution'),
      executor_type: z.enum(['claude-code', 'claude-ai', 'de-agent', 'human']).optional().describe('Override auto-detected executor type (only used with auto_dispatch)'),
      passphrase: z.string().optional().describe('Passphrase for write operations (required when WRITE_PASSPHRASE is configured)'),
    },
    async (args): Promise<CallToolResult> => {
      try {
        if (env.WRITE_PASSPHRASE && args.passphrase !== env.WRITE_PASSPHRASE) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Invalid passphrase' }) }],
            isError: true
          };
        ...
        // Auto-detect executor type from title prefix
        let executorType = args.executor_type;
        if (!executorType) {
          const titleLower = args.title.toLowerCase();
          if (titleLower.startsWith('[cc]') || titleLower.startsWith('[implement]') || titleLower.startsWith('[code]')) {
            executorType = 'claude-code';
          } else if (titleLower.startsWith('[research]') || titleLower.startsWith('[ai]')) {
            executorType = 'claude-ai';
          } else if (titleLower.startsWith('[human]') || titleLower.startsWith('[review]')) {
            executorType = 'human';
          } else {
            executorType = 'claude-ai'; // Default
          }
        }
        ...
        const importance = args.importance || 3;
        const urgency = args.urgency || 3;
        const priority = (importance * 2) + urgency; // Max 15

        await env.DB.prepare(`
          INSERT INTO tasks (
            id, tenant_id, user_id, title, description, status, due_date,
            importance, urgency, domain, energy_required, time_estimate_minutes,
            project_id, executor_type, source_type, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'mcp', ?, ?)
        `).bind(
          taskId, tenantId, userId,
          args.title, args.description || null,
          args.status || 'inbox', args.due_date || null,
          importance, urgency,
          args.domain || 'work', args.energy_required || 'medium',
          args.time_estimate_minutes || null, args.project_id || null,
          executorType, now, now
        ).run();

        let queueId: string | null = null;

        // Auto-dispatch if requested and status is 'next'
        if (args.auto_dispatch && (args.status === 'next' || !args.status)) {
          queueId = crypto.randomUUID();
          await env.DB.prepare(`
            INSERT INTO execution_queue (
              id, tenant_id, task_id, executor_type, status, priority, queued_at, context, created_at, updated_at
            ) VALUES (?, ?, ?, ?, 'queued', ?, ?, ?, ?, ?)
          `).bind(
            queueId, tenantId, taskId, executorType, priority, now,
            JSON.stringify({
              task_title: args.title,
              task_description: args.description || null,
              project_id: args.project_id || null,
              domain: args.domain || 'work',
              due_date: args.due_date || null,
              energy_required: args.energy_required || 'medium',
              source_type: 'mcp',
              source_reference: null,
            }),
            now, now
          ...
          // Update task status to 'next' if it wasn't already
          await env.DB.prepare(`
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              dispatched: !!queueId,
              message: queueId
                ? `Task created and queued for ${executorType} execution.`
                : `Task created. Use nexus_dispatch_task to queue for execution.`,
            }, null, 2)
          }]
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    async ({ executor_type, status, limit }): Promise<CallToolResult> => {
      try {
        const statusFilter = status === 'all' ? null : (status || 'queued');
        const maxResults = limit || 10;

        let query = `
          SELECT
            q.id, q.task_id, q.executor_type, q.status, q.priority,
            q.queued_at, q.claimed_at, q.claimed_by,
            q.attempt_count, q.max_attempts,
            t.title as task_title, t.description as task_description,
            q.context
          FROM execution_queue q
          JOIN tasks t ON q.task_id = t.id
          WHERE q.tenant_id = ? AND q.executor_type = ? AND q.deleted_at IS NULL
        `;

        const bindings: unknown[] = [tenantId, executor_type];
        ...
        query += ' ORDER BY q.priority DESC, q.queued_at ASC LIMIT ?';
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              queue: results.results.map((r: Record<string, unknown>) => ({
                id: r.id,
                task_id: r.task_id,
  ...
  server.tool(
    ...
    async ({ queue_id, claimed_by, passphrase }): Promise<CallToolResult> => {
      try {
        if (env.WRITE_PASSPHRASE && passphrase !== env.WRITE_PASSPHRASE) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Invalid passphrase' }) }],
            isError: true
          };
        ...
        const entry = await env.DB.prepare(`
          SELECT q.*, t.title as task_title, t.description as task_description
          FROM execution_queue q
          JOIN tasks t ON q.task_id = t.id
          WHERE q.id = ? AND q.tenant_id = ? AND q.deleted_at IS NULL
        `).bind(queue_id, tenantId).first();

        if (!entry) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Queue entry not found' }) }],
            isError: true
          };
        }
        ...
        if (entry.status !== 'queued') {
          return {
            content: [{ type: 'text', text: JSON.stringify({
              success: false,
              error: `Queue entry has status '${entry.status}', expected 'queued'`
            }) }],
            isError: true
          };
        }
        ...
        `).bind(now, claimed_by || 'unknown', now, queue_id).run();
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              context: entry.context ? JSON.parse(entry.context as string) : null,
              claimed_at: now,
              claimed_by: claimed_by || 'unknown',
            }, null, 2)
          }]
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    async ({ queue_id, result, error, auto_dispatch, passphrase }): Promise<CallToolResult> => {
      try {
        if (env.WRITE_PASSPHRASE && passphrase !== env.WRITE_PASSPHRASE) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Invalid passphrase' }) }],
        ...
        if (entry.status !== 'claimed') {
          return {
            content: [{ type: 'text', text: JSON.stringify({
              success: false,
              error: `Queue entry has status '${entry.status}', expected 'claimed'`
            }) }],
            isError: true
          };
        }
        ...
        if (isSuccess) {
          ...
          await env.DB.prepare(`
            ...
            WHERE id = ?
          `).bind(result || '{}', now, now, queue_id).run();
          ...
          // Failure - increment attempt count and maybe quarantine
          const failureHistory = entry.failure_history
            ? JSON.parse(entry.failure_history as string)
            : [];
          failureHistory.push({ error, timestamp: now });

          const newAttemptCount = (entry.attempt_count as number || 0) + 1;
          ...
          await env.DB.prepare(`
            ...
            SET status = 'failed',
                ...
                updated_at = ?
            WHERE id = ?
          `).bind(error, newAttemptCount, JSON.stringify(failureHistory), now, queue_id).run();

          // Update task failure count
          await env.DB.prepare(`
            UPDATE tasks SET failure_count = COALESCE(failure_count, 0) + 1, updated_at = ? WHERE id = ?
          `).bind(now, entry.task_id).run();

          // Check if should quarantine
          const quarantined = await checkAndQuarantine(env.DB, queue_id, tenantId);
          if (quarantined) {
            // Also quarantine the task
            await env.DB.prepare(`
              UPDATE tasks
              SET quarantine_status = 'quarantine',
                  ...
                  updated_at = ?
              WHERE id = ?
            `).bind(`Max attempts (${entry.max_attempts}) exceeded`, now, now, entry.task_id).run();
          }
        }
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              queue_id,
              task_id: entry.task_id,
              outcome: isSuccess ? 'completed' : 'failed',
              message: isSuccess
                ? 'Task completed successfully'
                : `Task failed (attempt ${(entry.attempt_count as number || 0) + 1}/${entry.max_attempts})`,
            }, null, 2)
          }]
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    async (): Promise<CallToolResult> => {
      try {
        ...
        // Also get quarantine count
        const quarantineCount = await env.DB.prepare(`
          SELECT COUNT(*) as count FROM execution_queue
          WHERE tenant_id = ? AND status = 'quarantine' AND deleted_at IS NULL
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              success: true,
              stats: stats.results,
              quarantine_count: quarantineCount?.count || 0,
            }, null, 2)
          }]
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    'List all quarantined tasks that have exceeded max failure attempts',
    {
    ...
    async ({ limit }): Promise<CallToolResult> => {
      try {
        const results = await env.DB.prepare(`
          ...
          FROM execution_queue q
          JOIN tasks t ON q.task_id = t.id
          WHERE q.tenant_id = ? AND q.status = 'quarantine' AND q.deleted_at IS NULL
          ORDER BY q.quarantined_at DESC
          LIMIT ?
        `).bind(tenantId, limit || 20).all();
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              success: true,
              count: results.results.length,
              quarantined: results.results.map((r: Record<string, unknown>) => ({
                queue_id: r.queue_id,
                task_id: r.task_id,
                task_title: r.task_title,
                executor_type: r.executor_type,
                ...
                failure_history: r.failure_history ? JSON.parse(r.failure_history as string) : [],
              })),
              message: results.results.length > 0
                ? 'Use nexus_unquarantine to retry or nexus_cancel_task to abandon.'
  ...
  // Tool: nexus_unquarantine
  server.tool(
    'nexus_unquarantine',
    'Remove a task from quarantine and optionally re-queue it for execution',
    {
      queue_id: z.string().uuid().describe('The quarantined queue entry ID'),
      requeue: z.boolean().optional().describe('If true, re-queue the task for execution (default: true)'),
      reset_attempts: z.boolean().optional().describe('If true, reset the attempt count (default: false)'),
      passphrase: z.string().optional().describe('Passphrase for write operations'),
    },
    async ({ queue_id, requeue, reset_attempts, passphrase }): Promise<CallToolResult> => {
      try {
        if (env.WRITE_PASSPHRASE && passphrase !== env.WRITE_PASSPHRASE) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Invalid passphrase' }) }],
        ...
        const entry = await env.DB.prepare(`
          SELECT * FROM execution_queue
          WHERE id = ? AND tenant_id = ? AND status = 'quarantine' AND deleted_at IS NULL
        `).bind(queue_id, tenantId).first();

        if (!entry) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Quarantined entry not found' }) }],
            isError: true
          };
        }

        const now = new Date().toISOString();
        const shouldRequeue = requeue !== false;
        const newStatus = shouldRequeue ? 'queued' : 'failed';
        ...
        await env.DB.prepare(`
          ...
          SET status = ?,
              ...
              updated_at = ?
          WHERE id = ?
        `).bind(newStatus, reset_attempts || false, now, queue_id).run();

        // Update task status
        await env.DB.prepare(`
          UPDATE tasks
          SET quarantine_status = 'resolved',
              status = CASE WHEN ? THEN 'next' ELSE 'waiting' END,
              updated_at = ?
          WHERE id = ?
        `).bind(shouldRequeue, now, entry.task_id).run();
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              attempts_reset: reset_attempts || false,
              message: shouldRequeue
                ? 'Task removed from quarantine and re-queued for execution.'
                : 'Task removed from quarantine but not re-queued.',
            }, null, 2)
          }]
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    {
      task_id: z.string().uuid().describe('Task ID to dispatch'),
      executor_type: z.enum(['claude-code', 'claude-ai', 'de-agent', 'human']).optional().describe('Override auto-detected executor type (auto-detects from title tag if not provided)'),
      passphrase: z.string().optional().describe('Passphrase for write operations (required when WRITE_PASSPHRASE is configured)'),
    },
    async ({ task_id, executor_type, passphrase }): Promise<CallToolResult> => {
      try {
        if (env.WRITE_PASSPHRASE && passphrase !== env.WRITE_PASSPHRASE) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Invalid passphrase' }) }],
        ...
        const existing = await env.DB.prepare(`
          SELECT id, status FROM execution_queue
          WHERE task_id = ? AND tenant_id = ? AND status NOT IN ('completed', 'failed', 'quarantine')
        `).bind(task_id, tenantId).first();
        ...
        if (existing) {
          return {
            content: [{ type: 'text', text: JSON.stringify({
              success: false,
              error: `Task already in queue with status '${existing.status}'`,
              queue_id: existing.id
            }) }],
            isError: true
          };
        ...
        // Determine executor type
        let execType = executor_type || task.executor_type;
        if (!execType) {
        ...
        const now = new Date().toISOString();
        const queueId = crypto.randomUUID();
        const priority = ((task.importance as number) || 3) * 2 + ((task.urgency as number) || 3);
        ...
        `).bind(
          queueId, tenantId, task_id, execType, priority, now,
          JSON.stringify({
            ...
            source_type: task.source_type || 'mcp',
        ...
        // Update task status
        await env.DB.prepare(`
          UPDATE tasks SET status = 'next', executor_type = ?, updated_at = ? WHERE id = ?
        `).bind(execType, now, task_id).run();
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              priority,
              message: `Task dispatched to ${execType} queue.`,
            }, null, 2)
          }]
        };
      } catch (error: any) {
  ...
  server.tool(
    ...
    'Dispatch all tasks with status="next" to the execution queue. Use this to immediately queue all ready tasks instead of waiting for the cron.',
    {
      executor_type: z.enum(['claude-code', 'claude-ai', 'de-agent', 'human']).optional().describe('Filter to only dispatch tasks that would route to this executor'),
      limit: z.number().optional().describe('Maximum number of tasks to dispatch (default: 50)'),
      passphrase: z.string().optional().describe('Passphrase for write operations (required when WRITE_PASSPHRASE is configured)'),
    },
    async ({ executor_type, limit, passphrase }): Promise<CallToolResult> => {
      try {
        if (env.WRITE_PASSPHRASE && passphrase !== env.WRITE_PASSPHRASE) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Invalid passphrase' }) }],
        ...
        // Get tasks with status 'next' that aren't already queued
        let query = `
          SELECT t.* FROM tasks t
          LEFT JOIN execution_queue q ON t.id = q.task_id AND q.status NOT IN ('completed', 'failed', 'quarantine')
          WHERE t.tenant_id = ? AND t.status = 'next' AND t.deleted_at IS NULL AND q.id IS NULL
        `;
        const bindings: unknown[] = [tenantId];

        if (executor_type) {
          query += ' AND (t.executor_type = ? OR t.executor_type IS NULL)';
          bindings.push(executor_type);
        }
        ...
        const now = new Date().toISOString();
        const dispatched: { task_id: string; queue_id: string; executor_type: string }[] = [];

        for (const task of tasks.results as Record<string, unknown>[]) {
          // Determine executor type
          let execType = task.executor_type as string;
          if (!execType) {
            const titleLower = (task.title as string).toLowerCase();
            if (titleLower.startsWith('[cc]') || titleLower.startsWith('[implement]') || titleLower.startsWith('[code]')) {
              execType = 'claude-code';
          ...
          // Skip if filtering by executor and doesn't match
          if (executor_type && execType !== executor_type) continue;
          ...
          `).bind(
            queueId, tenantId, task.id, execType, priority, now,
            JSON.stringify({
              task_title: task.title,
              task_description: task.description,
              project_id: task.project_id || null,
          ...
          // Update task with executor type
          await env.DB.prepare(`
            UPDATE tasks SET executor_type = ?, updated_at = ? WHERE id = ?
          `).bind(execType, now, task.id).run();

          dispatched.push({ task_id: task.id as string, queue_id: queueId, executor_type: execType });
        }
          ...
          content: [{
            ...
            text: JSON.stringify({
              ...
              message: dispatched.length > 0
                ? `Dispatched ${dispatched.length} tasks to execution queue.`
